...
public class ConduitShapeHelper {
    ...
    public static Vector3f[] segmentOffsets(final int numSegments, @Nullable final Direction dir) {
        final Vector3f[] offsets = switch (numSegments) {
            ...
            case 3 ->
                    new Vector3f[]{new Vector3f(1.75F / 16F, 0, -1.75F / 16F), new Vector3f(-1.75F / 16F, 0, -1.75F / 16F), new Vector3f(0, 0, 1.75F / 16F)};
    ...
    public static AABB boundMixedJoint(final Map<Direction, Integer> connections) {
        ...
        final FloatFloatPair x = bounds.keySet().stream().filter(Direction.Axis.X).map(bounds::get)
                .reduce(ConduitUtil::max).orElse(one);
        final FloatFloatPair y = bounds.keySet().stream().filter(Direction.Axis.Y).map(bounds::get)
                .reduce(ConduitUtil::max).orElse(one);
        final FloatFloatPair z = bounds.keySet().stream().filter(Direction.Axis.Z).map(bounds::get)
                .reduce(ConduitUtil::max).orElse(one);
