...
public class NBTUtil {
    ...
    public static <T, V> V encode(final DynamicOps<? extends V> ops, final T data, final Codec<? super T> codec) {
        ...
        result.ifError(err -> {
            throw new IllegalArgumentException(String.format("Failed to encode: %s", err.message()));
        });
    ...
    public static <T, V> T decode(final DynamicOps<V> ops, final V data, final Codec<? extends T> codec) {
        ...
        result.ifError(err -> {
            throw new IllegalArgumentException(String.format("Failed to decode: %s", err.message()));
        });
        return result.getOrThrow();
    }
